	...
	void Update()
	{
		...
		if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, 50))
		{
			...
			if (hit.transform.gameObject.CompareTag("Player"))
			{
				...
				if (counter < 0 && hit.transform.gameObject.name == "MapaDoDino")
				{
					SceneManager.LoadScene("Mapa Dinos");
					//hit.transform.gameObject.SendMessageUpwards("ButtonAction");
					counter = 3;//reseta o contador
				}
				if (counter < 0 && hit.transform.gameObject.name == "DragaoWOW")
				{
					SceneManager.LoadScene("Mapa Dragões");
					//hit.transform.gameObject.SendMessageUpwards("ButtonAction");
					counter = 3;//reseta o contador
				}
				if (counter < 0 && hit.transform.gameObject.name == "CUBISMO")
				{
					SceneManager.LoadScene("Mapa Cubo");
					//hit.transform.gameObject.SendMessageUpwards("ButtonAction");
					counter = 3;//reseta o contador
				}
				if (counter < 0 && hit.transform.gameObject.name == "MENU")
				{
